...
`fewshot.jsonl` (39 ví dụ) là thứ lấp vào **quyết định treo D1** ở [`PHASE2_PLAN.md`](PHASE2_PLAN.md) §1.
...
trừ nhau không*. Hai construct khác nhau. Xem `fewshot_draft.jsonl`, trường `_agrees_with_machine`.
...
lập được theo seed. Bản gốc ở `track_b_pipeline.ipynb.bak`.

## Dựng lại

```powershell
python phase-2/build_fewshot_candidates.py   # 36 ứng viên + nhãn máy suy từ intensity
python phase-2/apply_fewshot_draft.py        # pass đọc tay, kèm cờ lệch với nhãn máy
python phase-2/build_fewshot_final.py        # -> phase-2/fewshot.jsonl
```
